...
import { useMiner } from '@/data/queries'
      ...
      <div className="page maxi">
        <section className="maxi-bar">
          <ul className="maxi-tools">
            {maximizer.tools.isLoading
              ...
              : tools.map((tool) => (
                  <li key={tool.asset_id} className="maxi-tool" style={{ '--rarity': RARITY_COLORS[tool.rarity] } as CSSProperties}>
        ...
        <section className="maxi-planets" aria-busy={maximizer.isLoading}>
          {maximizer.isLoading
            ...
            : maximizer.planets.map(({ row, value }) => (
                <article key={row.asset_id} className={`maxi-planet ${maximizer.best?.row.asset_id === row.asset_id ? 'is-best' : ''}`}>
        ...
        <div className="maxi-actions">
          <Button
            size="lg"
            color="gradientPink"
            disabled={!account || maximizer.isLoading || miner.isLoading || tools.length === 0 || !ready}
