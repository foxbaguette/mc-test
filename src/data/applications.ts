...
export function usePendingApplications() {
  const members = useQuery({ queryKey: ['applications', 'members'], queryFn: readAllMembers, staleTime: 2 * MIN })
...
export const useFlagReasons = () =>
  useQuery({ queryKey: ['applications', 'reasons'], queryFn: readFlagReasons, staleTime: 60 * MIN })

/** What support did with this member before. */
export const useSupportLog = (wallet: string) =>
  useQuery({ queryKey: ['applications', 'suplog', wallet], queryFn: () => readSupportLog(wallet), staleTime: 5 * MIN })

export const refreshApplications = () => queryClient.invalidateQueries({ queryKey: ['applications'] })
